...
namespace ProductionProjectWebsite.Account
{
    ...
    // Kassy Zoschke/Dean Jones
    // January 23, 2017
    // Page used to connect to database to retrieve package info
    //**************************************
    [DataObject(true)]
    public static class PackageDB
    {
        ...
        public static List<Package> GetPackage()
        {
            ...
            // get selectString
            string selectString = "Select PackageId, PkgName, PkgStartDate, PkgEndDate, PkgDesc, PkgBasePrice from Packages WHERE PkgStartDate > CONVERT(DATETIME, @nowdate) " +
                                  "order by PkgName";
            SqlCommand selectCommand = new SqlCommand(selectString, connection);
            selectCommand.Parameters.AddWithValue("@nowdate", DateTime.Today.ToString());
            try
            {
                connection.Open();
...
/*
 * Author: Mark Maillet
 * Abstract: This method returns the packages for a specified customer
 */
        [DataObjectMethod(DataObjectMethodType.Select)]
        public static List<Package> GetCustomerPackages(Customer c)
        {
            ...
            try
            {
                ...
                {
                    conn.Open();
                    SqlCommand cmd = new SqlCommand("SELECT pa.PackageId,pa.PkgName,pa.PkgStartDate,pa.PkgEndDate,pa.PkgDesc,pa.PkgBasePrice FROM Packages pa " +
